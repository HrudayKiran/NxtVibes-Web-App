...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: profileError } = await supabase.from("profiles").insert([
        {
          id: data.user.id,
          name: name.trim(),
          username: email.trim().split("@")[0] + "_" + Math.floor(Math.random() * 1000),
          photo_url: null,
          bio: "",
        },
      ]);
      ...
      toast({
        ...
        description: "Welcome to NxtVibes! Let's complete your profile.",
        variant: "success",
      });
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center p-4 bg-background font-sans">
      ...
      <div className="absolute top-1/4 right-1/4 h-[300px] w-[300px] rounded-full bg-primary/20 blur-[100px]" />
      <div className="absolute bottom-1/4 left-1/4 h-[300px] w-[300px] rounded-full bg-secondary/15 blur-[100px]" />
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-border bg-card p-8 shadow-premium md:p-10">
        <div className="flex flex-col items-center gap-2 mb-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">Sign up to start planning your next trip</p>
        </div>
